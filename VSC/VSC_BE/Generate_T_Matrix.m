...
n_Sim_input=n_state+n_input;   %% BE method: [x_his, u_now]
n_Sim_output=n_state+n_output;   %% BE method: [x_now, y_now]
...
% BE method
tic;
trig_now_select = [trig_now_select(1,:); trig_now_select(1:n_select-1,:)]; % In simulink，triggers signals are used in the nex step
trig_dec = arrayfun(@(row) bin2dec(num2str(trig_now_select(row, :), '%d')), 1:n_select)'; % Convert binary to decimal
...
for i = 1:n_Combination
    ...
    Input_Data = [x_select(idx_mid-1, :),u_select(idx_mid, :)];
...
for i=1:n_tot
    ...
    Tehis=T(idx_yvi_ex,1:n_state);
    Tee=T(idx_yvi_ex,n_state+idx_uvi_ex);  
    Tei=T(idx_yvi_ex,n_state+idx_uvi_in);
    Vin_now=Vin(i,:)';
    Vex_now=Vex(i,:)';

    His_source=Tehis*Sim_input(1:n_state)+Tei*Vin_now;
    H_eq=Tee+Gex;
    VIeq=Gex*Vex_now-His_source;
    VIcal=inv(H_eq)*VIeq;  

    Sim_input((n_state+1):n_Sim_input)=[VIcal;Vin_now]; % update u_now

    % Update using T matrix
