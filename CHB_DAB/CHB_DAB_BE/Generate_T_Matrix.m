...
n_Sim_input=n_state+n_input;   %% BE method: [x_last, u_now]
...
tic; 
sim('.\Generate_T_Matrix_CHB_DAB.slx'); % Run the Simulink model
...
for i = 1:n_Combination
    ...
    fprintf('Combination %d: %s (%d), Number of points: %d\n', i, dec2bin(Trigger_value(i),n_trig),Trigger_value(i),n_Combination_point(i));
end
fprintf('\n');
...
for i=1:n_tot
    ...
    Tee=T(n_state+1:n_state+2,n_state+1:n_state+2);
    Tehis=T(n_state+1:n_state+2,1:n_state);   
    His=Tehis*Sim_input(1:n_state);
    ...
    Sim_input((n_state+1):n_Sim_input)=VIcal;% u_now
